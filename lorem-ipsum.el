...
(defconst lorem-ipsum
  ...
  "The original lorem ipsum paragraphs.")

(defvar lorem-ipsum-sentence-seperator " "
  "Separator to place between sentences when inserting lorem ipsum.")

(defvar lorem-ipsum-paragraph-seperator "\n\n"
  "Separator to place between paragraphs when inserting lorem ipsum.")

(defun lorem-ipsum--random-paragraph ()
  "Select a random paragraph."
  (mapconcat 'identity (nth (random (length lorem-ipsum)) lorem-ipsum)
             lorem-ipsum-sentence-seperator))

(defun lorem-ipsum--random-sentence ()
  "Select a random sentence."
  (let ((para (nth (random (length lorem-ipsum)) lorem-ipsum)))
    (nth (random (length para)) para)))

(defun lorem-ipsum--generate-paragraph ()
  "Generate a brand new paragraph."
  (mapconcat 'identity
             (loop for i from 1 to (+ 10 (random 2))
                   collect (lorem-ipsum--random-sentence))
             lorem-ipsum-sentence-seperator))

(defun lorem-ipsum-insert-paragraphs (n)
  "Insert prefix-argument number paragraphs at point."
  (interactive "p")
  (insert (mapconcat 'identity (car lorem-ipsum) lorem-ipsum-sentence-seperator)
          lorem-ipsum-paragraph-seperator)
  (dotimes (i (1- n))
    (insert (lorem-ipsum--generate-paragraph)
            lorem-ipsum-paragraph-seperator)))

(defun lorem-ipsum-insert-sentences (n)
  "Insert prefix-argument number sentences at point."
  (interactive "p")
  (dotimes (i n)
    (insert (lorem-ipsum--random-sentence) lorem-ipsum-sentence-seperator)))
